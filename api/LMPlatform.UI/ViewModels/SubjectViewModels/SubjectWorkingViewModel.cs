...
namespace LMPlatform.UI.ViewModels.SubjectViewModels
{
    public class SubjectWorkingViewModel
    {
        ...
        {
            SubjectId = subjectId;
			Subject = SubjectManagementService.GetSubject(new Query<Subject>(e => e.Id == subjectId).Include(e => e.SubjectModules.Select(x => x.Module)).Include(e => e.SubjectLecturers.Select(x => x.Lecturer.User)));
