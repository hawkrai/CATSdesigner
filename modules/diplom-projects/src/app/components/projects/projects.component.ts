...
export class ProjectsComponent implements OnInit {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        ...
        message: this.translatePipe.transform(
          'text.editor.edit.check',
          'Вы действительно хотите выбрать данную тему дипломного проекта?'
        ),
        actionName: this.translatePipe.transform(
          'text.editor.edit.choose',
          'Выбрать'
        ),
        color: 'primary',
      },
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != null && result) {
        this.projectsService.chooseProject(projectId).subscribe(() => {
          ...
          this.addFlashMessageSuccess(
            this.translatePipe.transform(
              'text.editor.edit.response',
          ...
          this.projectsService
            ...
            .subscribe(
              () => {
                ...
                this.addFlashMessageSuccess(
                  this.translatePipe.transform(
                    'text.editor.edit.responseSave',
                    'Тема успешно сохранена'
                  )
                )
              },
              () =>
                this.addFlashMessageError(
                  this.translatePipe.transform(
                    'text.editor.edit.responseError',
      ...
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
        ...
        data: {
          label: this.translatePipe.transform(
            ...
            'Удаление темы дипломного проекта'
          ),
          message: this.translatePipe.transform(
            'text.diplomProject.removeThemeQuestion',
            'Вы действительно хотите удалить тему дипломного проекта?'
          ),
          actionName: this.translatePipe.transform(
            'text.diplomProject.removeButton',
            'Удалить'
          ),
          color: 'primary',
        },
      ...
      dialogRef.afterClosed().subscribe((result) => {
        if (result != null && result) {
          this.projectsService.deleteProject(project.Id).subscribe(() => {
            ...
            this.addFlashMessageSuccess(
              this.translatePipe.transform(
                'text.editor.edit.responseDelete',
          ...
          if (result != null && result) {
            this.projectsService
              ...
              .subscribe(() => {
                ...
                this.addFlashMessageSuccess(
                  this.translatePipe.transform(
                    'text.editor.edit.assignThemeForStudent',
  ...
  approveChoice(project: Project) {
    this.projectsService.approveChoice(project.Id).subscribe(() => {
      ...
      this.addFlashMessageSuccess(
        this.translatePipe.transform(
          'text.editor.edit.responseAssign',
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      data: {
        label: this.translatePipe.transform(
          ...
          'Отменить назначение темы дипломного проекта'
        ),
        message: this.translatePipe.transform(
          'text.diplomProject.cancelAssignQuestion',
          'Вы действительно хотите отменить назначение темы дипломного проекта?'
        ),
        actionName: this.translatePipe.transform(
          'text.diplomProject.removeAssign',
          'Убрать назначение'
        ),
        color: 'primary',
      },
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != null && result) {
        this.projectsService.removeAssignment(project.Id).subscribe(() => {
          ...
          this.addFlashMessageSuccess(
            this.translatePipe.transform(
              'text.editor.edit.responseRemove',
