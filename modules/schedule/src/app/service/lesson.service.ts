...
export class LessonService {
  lessonTypes: string[][] = [
    ['0', this.translate.transform('text.schedule.lecture.cut', 'Лекция')],
    [
      '1',
      this.translate.transform('text.schedule.workshop.cut', 'Практ. зан.'),
    ],
    ...
    [
      '4',
      this.translate.transform('text.schedule.graduation.project.cut', 'ДП'),
    ],
  ]
  ...
  getAllLessons(username: string): Observable<any> {
    return this.http.post<any>('/Profile/GetProfileInfoCalendar', {
      userLogin: username,
    })
  }
  ...
  getLessonsByDates(start: string, end: string): Observable<any> {
    return this.http.get<any>(
      '/Services/Schedule/ScheduleService.svc/GetSchedule?dateStart=' +
        start +
        '&dateEnd=' +
        end
    )
  ...
  deleteLab(idLab: any, subjId: any): Observable<any> {
    return this.http.post<any>(
      '/Services/Schedule/ScheduleService.svc/DeleteLabScheduleDate',
      { id: idLab, subjectId: subjId }
    )
  }
  ...
  savePractical(pract: any, datePr: string): Observable<any> {
    return this.http.post<any>(
      ...
      {
        id: pract.Id,
        subjectId: pract.SubjectId,
        date: datePr,
        startTime: pract.Start,
        endTime: pract.End,
  ...
  deletePractical(idPract: any, subjId: any): Observable<any> {
    return this.http.post<any>(
      '/Services/Schedule/ScheduleService.svc/DeletePracticalScheduleDate',
      { id: idPract, subjectId: subjId }
    )
  }
  ...
  deleteLecture(lectId: any, subjId: any): Observable<any> {
    return this.http.post<any>(
      '/Services/Schedule/ScheduleService.svc/DeleteLectureScheduleDate',
      { id: lectId, subjectId: subjId }
    )
  }

  getLessonsByDateAndTimes(
    date: string,
    start: string,
    end: string
  ): Observable<any> {
    return this.http.get<any>(
      '/Services/Schedule/GetScheduleBetweenTime?date=' +
        date +
        '&startTime=' +
        start +
        '&endTime=' +
  ...
  getSubjectOwner(subjId: any): Observable<any> {
    return this.http.get<any>(
      '/Services/Subjects/SubjectsService.svc/GetSubjectOwner/' + subjId
    )
  }
