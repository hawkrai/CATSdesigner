...
export class SelectEventTypeComponent implements OnInit {
  ...
  select() {
    if (this.type === '1') {
      const dialogRef = this.dialog.open(CreateLessonComponent, {width: '300px', data: {userName: this.data.userName}});
      ...
      const dialogRef = this.dialog.open(AddNoteComponent, {width: '300px', data: { event: null}, position: {top: '11%'}});
      dialogRef.afterClosed().subscribe(result => {
        if (result != null) {
