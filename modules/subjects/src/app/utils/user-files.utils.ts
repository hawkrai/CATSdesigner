...
export const sendUserFileSuccess = (
  ...
  if (index >= 0) {
    return userLabFiles
      .slice(0, index)
      .concat(userLabFile, userLabFiles.slice(index + 1))
  }
...
const sortFiles = (files: UserLabFile[]): UserLabFile[] => {
  const comparisonFn = (aStringDate: string, bStringDate: string) => {
    const aDate = aStringDate
      ? moment(aStringDate, 'DD.MM.yyyy HH:mm').toDate()
      : null
    const bDate = aStringDate
      ? moment(bStringDate, 'DD.MM.yyyy HH:mm').toDate()
      : null
