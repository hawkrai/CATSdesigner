...
export const subjectReducer = createReducer(
  ...
  on(subjectActions.loadSubjectSuccess, (state, { subject }): ISubjectState => ({
    ...state,
...
const onSubjectLodded = (subjects: Subject[], subject: Subject) => {
  ...
  const subjectIndex = subjects.findIndex(x => x.SubjectId == subject.SubjectId)
  if (subjectIndex >= 0) {
    return subjects.slice(0, subjectIndex).concat(subject, subjects.slice(subjectIndex + 1));
  }
