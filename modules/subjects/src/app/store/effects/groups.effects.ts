...
export class GroupsEffects {
  ...
  loadActiveGroups$ = createEffect(() =>
    this.actions$.pipe(
      ofType(groupsActions.loadActiveGroups),
      switchMap(({ groupId }) =>
        this.store.select(getSubjectId).pipe(
          switchMap((subjectId) => this.rest.getAllGroups(subjectId)),
          switchMap(({ groups, hasInactiveGroups }) => [
            groupsActions.loadGroupsSuccess({ groups, groupId }),
            groupsActions.checkHasInactiveSuccess({ hasInactiveGroups }),
          ])
        )
      )
    )
  ...
  loadStudentGroup$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, subjectId, userId]) =>
        this.rest
          .getUserSubjectGroup(subjectId, userId)
          .pipe(map((group) => groupsActions.setCurrentGroup({ group })))
      )
    )
  )

  triggerGridActiveState$ = createEffect(() =>
    this.actions$.pipe(
  ...
  loadGroupsTrigger$ = createEffect(() =>
    this.actions$.pipe(
  ...
  loadGroups$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(this.store.select(groupsSelectors.isActiveGroup)),
      map(([{ groupId }, isActive]) =>
        isActive
          ? groupsActions.loadActiveGroups({ groupId })
          : groupsActions.loadOldGroups({ groupId })
      )
    )
  )
}
