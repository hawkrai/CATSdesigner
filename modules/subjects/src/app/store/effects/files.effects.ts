...
export class FilesEffects {
  ...
  loadAttachments$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ values }) =>
        this.filesService
          .getAttachments({ values, deleteValues: 'DELETE' })
          .pipe(map((files) => filesActions.loadAttachmentsSuccess({ files })))
      )
    )
  )
  ...
  getAttachemntsAsZip$ = createEffect(
    () =>
      this.actions$.pipe(
        ofType(filesActions.getAttachmentsAsZip),
        tap(() => {
          this.store.dispatch(
            filesActions.setIsDownloading({ isDownloading: true })
          )
        }),
        switchMap(({ attachmentsIds }) =>
          this.subjectService.getAttachmentsAsZip(attachmentsIds)
        ),
        tap((response) => {
          this.store.dispatch(
            filesActions.setIsDownloading({ isDownloading: false })
          )
          this.coreService.download(response)
        })
      ),
