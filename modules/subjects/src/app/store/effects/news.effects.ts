...
export class NewsEffects {
  ...
  getNews$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, subjectId]) =>
        this.rest
          .getAllNews(subjectId)
          .pipe(map((news) => newsActions.loadNewsSuccess({ news })))
      )
    )
  )

  disableNews$ = createEffect(() =>
    this.actions$.pipe(
      ofType(newsActions.disableAllNews),
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      mergeMap(([_, subjectId]) =>
        this.rest.disableNews(subjectId).pipe(map(() => newsActions.loadNews()))
      )
    )
  )

  enableNews$ = createEffect(() =>
    this.actions$.pipe(
      ofType(newsActions.enableAllNews),
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      mergeMap(([_, subjectId]) =>
        this.rest.enableNews(subjectId).pipe(map(() => newsActions.loadNews()))
      )
    )
  )

  saveNews$ = createEffect(() =>
    this.actions$.pipe(
      ofType(newsActions.saveNews),
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      mergeMap(([{ news }, subjectId]) =>
        this.rest
          .saveNews({ ...news, subjectId })
          .pipe(map(() => newsActions.loadNews()))
      )
    )
  )
  ...
  deleteNews$ = createEffect(() =>
    this.actions$.pipe(
      ...
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      mergeMap(([{ id }, subjectId]) =>
        this.rest
          .deleteNews(id, subjectId)
          .pipe(map(() => newsActions.loadNews()))
      )
    )
  )
