...
export class PracticalsEffects {
  ...
  updateOrder$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([{ prevIndex, currentIndex }, subjectId]) =>
        this.rest
          .updatePracticalsOrder(subjectId, prevIndex, currentIndex)
          .pipe(map(() => practicalsActions.loadPracticals()))
      )
    )
  )

  deletePractical$ = createEffect(() =>
    this.actions$.pipe(
  ...
  createPractical$ = createEffect(() =>
    this.actions$.pipe(
      ofType(practicalsActions.savePractical),
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      switchMap(([{ practical }, subjectId]) =>
        this.rest
  ...
  loadPracticalsVisitingMarks$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, groupId, subjectId]) =>
        this.rest
          .getMarks(subjectId, groupId)
          .pipe(
            switchMap(({ students, testsCount }) => [
              practicalsActions.loadMarksSuccess({ students }),
  ...
  setPracticalsVisitingDate$ = createEffect(() =>
    this.actions$.pipe(
      ofType(practicalsActions.setPracticalsVisitingDate),
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      switchMap(([{ visiting }, subjectId]) =>
        this.rest
  ...
  setPracticalMark$ = createEffect(() =>
    this.actions$.pipe(
      ofType(practicalsActions.setPracticalMark),
      withLatestFrom(this.store.select(subjectSelectors.getSubjectId)),
      switchMap(([{ body }, subjectId]) =>
        this.rest
  ...
  practicalsVisitingExcel$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, subjectId, groupId]) =>
        this.rest
          .getVisitPrcaticalsExcel(subjectId, groupId)
          .pipe(map((response) => filesActions.exportFile({ response })))
      )
    )
  )

  practicalsMarksExcel$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, subjectId, groupId]) =>
        this.rest
          .getPracticalsMarksExcel(subjectId, groupId)
          .pipe(map((response) => filesActions.exportFile({ response })))
      )
    )
  )
    ...
    this.actions$.pipe(
      ...
      switchMap(({ userLabFileId, userId }) =>
        this.userFilesService
          ...
          .pipe(
            map(() =>
              practicalsActions.deleteUserFileSuccess({ userId, userLabFileId })
            )
          )
      )
    )
      ...
      switchMap(({ userId, userFileId }) =>
        this.userFilesService
          ...
          .pipe(
            switchMap((body) => [
              catsActions.showMessage({ body }),
              practicalsActions.receiveFileSuccess({ userId, userFileId }),
            ])
          )
      )
    )
  ...
  checkJobProptection$ = createEffect(() =>
    this.actions$.pipe(
      ofType(
        ...
        practicalsActions.protectionChangedUpdate,
        practicalsActions.sendUserFileSuccess
      ),
      withLatestFrom(
      ...
      switchMap(
        ([{ userId, type }, currentUserId, subjectId, groupId, isTeacher]) => {
          if (type === practicalsActions.sendUserFileSuccess.type) {
            return [
              protectionActions.protectionChanged({
                ...
                subjectId,
                groupId,
                protectionType: ProtectionType.Practical,
              }),
            ]
          }
          ...
          if (isTeacher) {
            actions.push(
              practicalsActions.loadStudentJobProtection({ studentId: userId })
            )
          } else {
      ...
      switchMap(({ userId, userFileId }) =>
        this.userFilesService
          ...
          .pipe(
            switchMap((body) => [
              catsActions.showMessage({ body }),
              practicalsActions.returnFileSuccess({ userId, userFileId }),
            ])
          )
      )
    )
  ...
  loadStudentFiles$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([{ userId }, currentUserId, subjectId]) =>
        this.rest
          .getUserPracticalFiles(!!userId ? userId : currentUserId, subjectId)
          .pipe(
            map((practicalFiles) =>
      ...
      switchMap(([_, subjectId, isTeacher, isActive]) =>
        iif(
          ...
          map((hasJobProtections) =>
            practicalsActions.setJobProtections({ hasJobProtections })
          )
        )
      )
    )
  ...
  getAsZip$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(([_, groupId, subjectId]) =>
        this.rest
          .getPracticalsZip(subjectId, groupId)
          .pipe(map((response) => filesActions.exportFile({ response })))
      )
    )
  )

  cancelFile$ = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap(({ userId, userFileId }) =>
        this.userFilesService
          ...
          .pipe(
            switchMap((body) => [
              catsActions.showMessage({ body }),
              practicalsActions.cancelFileSuccess({ userId, userFileId }),
            ])
          )
      )
    )
  ...
  protectionChanged$ = createEffect(() =>
    this.actions$.pipe(
      ...
      map((body) =>
        practicalsActions.protectionChangedUpdate({ userId: body.userId })
      )
    )
  )
}
