  ...
  implements OnInit, OnDestroy, AfterViewChecked
{
  ...
  ngOnInit(): void {
    ...
    this.subs.add(
      this.store.select(lecturesSelectors.getLectures).subscribe((lectures) => {
    ...
    const dialogData: DialogData = {
      title: lecture
        ? this.translate.transform(
            'text.subjects.lectures.editing',
            'Редактирование темы лекции'
          )
        : this.translate.transform(
            'text.subjects.lectures.adding',
            'Добавление темы лекции'
          ),
  ...
  deleteLectures(lecture: Lecture) {
    const dialogData: DialogData = {
      title: this.translate.transform(
        'text.subjects.lectures.deleting',
        'Удаление лекции'
      ),
      body:
        `${this.translate
          .transform('text.lectures.accusative', 'Лекцию')
          .toLowerCase()} "` +
        lecture.Theme +
        '"',
    ...
    this.subs.add(
      dialogRef.afterClosed().subscribe((result) => {
        if (result) {
          this.store.dispatch(
            lecturesActions.deleteLecture({ id: lecture.LecturesId })
          )
        }
      })
    )
