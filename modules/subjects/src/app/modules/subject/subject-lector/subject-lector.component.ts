...
export class SubjectLectorComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.store.select(subjectSelectors.getUserId).subscribe((userId) => {
      ...
      if (this.data.model && this.data.model.subjectId) {
        ...
        this.store
          .select(subjectSelectors.getSelectedSubject)
          .subscribe((subject) => {
  ...
  setLectors() {
    ...
    this.subjectService
      .getNoAdjointLectors(this.subjectId)
      .subscribe((lectors) => {
  ...
  joinLector() {
    this.subjectService
      .joinedLector(this.subjectId, this.selectedLector.LectorId)
      .subscribe((body) => {
        if (body.Code === '200') {
          this.joinedLectors = this.sortLectors([
            ...this.joinedLectors,
            this.selectedLector,
          ])
          this.allLectors = this.sortLectors(
            this.allLectors.filter(
              (x) => x.LectorId !== this.selectedLector.LectorId
            )
          )
  ...
  disjoinLector(lector) {
    this.subjectService
      .disjoinLector(this.subjectId, lector.LectorId)
      .subscribe((body) => {
        if (body.Code === '200') {
          this.joinedLectors = this.sortLectors(
            this.joinedLectors.filter((x) => x.LectorId !== lector.LectorId)
          )
