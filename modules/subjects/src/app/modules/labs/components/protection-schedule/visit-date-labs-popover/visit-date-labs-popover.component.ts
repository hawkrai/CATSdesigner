...
export class VisitDateLabsPopoverComponent implements OnInit {
  ...
  ngOnInit() {
    this.state$ = combineLatest(
      this.store
        .select(labsSelectors.getLabsCalendar)
        .pipe(
          map((schedule) =>
            schedule.filter(
              (day) => day.SubGroupId === this.data.body.subGroupId
            )
          )
        ),
      this.store
  ...
  onDeleteDay(day: ScheduleProtectionLab): void {
    this.store.dispatch(
      labsActions.deleteDateVisit({ id: day.ScheduleProtectionLabId })
    )
  }
}
