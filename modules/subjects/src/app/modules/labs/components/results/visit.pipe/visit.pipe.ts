...
export class VisitPipe implements PipeTransform {
  transform(value: StudentMark, ...args: [string, Lab[]]): LabVisitingMark[] {
    const lab = args[1].find(
      (res) =>
        res.LabId.toString() === args[0].toString() &&
        res.SubGroup === value.SubGroup
    )
    const scheduleIds = []
    ...
    return value.LabVisitingMark.filter(
      (visiting) =>
        scheduleIds.includes(visiting.ScheduleProtectionLabId.toString()) &&
        visiting.Mark
    )
  }
}
