...
export class JobProtectionComponent implements OnDestroy {
  ...
  addPractical({ userId, file, practicalId, fileId }: { userId: number, file: UserLabFile, practicalId?: number, fileId?: number }): void {
    const dialogData: DialogData = {
      title: this.isTeacher ? this.translate.transform('text.subjects.labs.for.revision', 'На доработку') : this.translate.transform('text.subjects.practicals.to.protect', 'На защиту практической работы'),
    ...
    const dialogRef = this.dialogService.openDialog(AddPracticalPopoverComponent, dialogData);

    this.subs.add(
      dialogRef.afterClosed().pipe(
        filter(data => !!data),
        withLatestFrom(this.store.select(subjectSelectors.getUserId)),
        map(([data, currentUserId]: [{ comments: string, attachments: Attachment[], practicalId: number }, number]) => this.getSendFile(file, data, userId ? userId : currentUserId, !!practicalId))
      ).subscribe(sendFile => {

        this.store.dispatch(practicalsActions.sendUserFile({ sendFile, fileId }));
      })
    );
  }
  ...
  deletePractical(deleteRequest: { userLabFileId: number, userId: number, practicalId: number }): void {
    const dialogData: DialogData = {
  ...
  getSendFile(file: UserLabFile, data: { comments: string, attachments: Attachment[], practicalId: number }, userId: number, isReturn: boolean = false) {
    return {
