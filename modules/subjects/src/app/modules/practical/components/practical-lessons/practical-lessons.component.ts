...
export class PracticalLessonsComponent
    ...
    const dialogData: DialogData = {
      title: lesson
        ? this.translate.transform(
            'text.subjects.practicals.editing',
            'Редактирование практического занятия'
          )
        : this.translate.transform(
            'text.subjects.practicals.adding',
    ...
    this.subs.add(
      dialogRef
        ...
        .subscribe((result) => {
          ...
          this.store.dispatch(
            practicalsActions.savePractical({
              practical: result as CreateLessonEntity,
            })
          )
        })
    )
  ...
  deleteLesson(lesson: Practical) {
    const dialogData: DialogData = {
      title: this.translate.transform(
        'text.subjects.practicals.deleting',
        'Удаление практического занятия'
      ),
      body: `${this.translate
        .transform('text.subjects.practicals', 'практическое занятие')
        .toLowerCase()} "${lesson.Theme}"`,
    ...
    this.subs.add(
      dialogRef
        ...
        .subscribe(() => {
          this.store.dispatch(
            practicalsActions.deletePractical({ id: lesson.PracticalId })
          )
        })
    )
  }
