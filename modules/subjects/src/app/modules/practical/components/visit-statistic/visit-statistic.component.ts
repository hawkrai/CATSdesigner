...
export class VisitStatisticComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.state$ = combineLatest([
      ...
      map(([practicals, schedule, students, userId, isTeacher]) => ({
        practicals,
        schedule,
        students,
        userId,
  ...
  getDisplayedColumns(schedule: ScheduleProtectionPractical[]): string[] {
    return ['position', 'name'].concat(
      ...schedule.map((s) => s.Date + s.ScheduleProtectionPracticalId)
    )
  }
  ...
  setVisitMarks(
    ...
    const visits = {
      ...
      students: students.map((s) => {
        const practiacalVisitingMark = s.PracticalVisitingMark.find(
          (x) =>
            x.ScheduleProtectionPracticalId ===
            schedule.ScheduleProtectionPracticalId
        )
        return {
          name: s.FullName,
          mark: practiacalVisitingMark ? practiacalVisitingMark.Mark : '',
          comment: practiacalVisitingMark ? practiacalVisitingMark.Comment : '',
          showForStudent: practiacalVisitingMark
            ? practiacalVisitingMark.ShowForStudent
            : false,
        }
      }),
    }
    ...
    dialogRef
      ...
      .subscribe((visiting) => {
        this.store.dispatch(
          practicalsActions.setPracticalsVisitingDate({ visiting })
        )
      })
  }
  ...
  findVisitingIndex(
    ...
    return studentMark.PracticalVisitingMark.findIndex(
      (x) =>
        x.ScheduleProtectionPracticalId ===
        schedule.ScheduleProtectionPracticalId
    )
  ...
  private getVisitingPracticals(
    ...
    students.forEach((student) => {
      const visitingMark = student.PracticalVisitingMark.find(
        (x) => x.ScheduleProtectionPracticalId === dateId
      )
      visitsModel.Id.push(
        visitingMark ? visitingMark.PracticalVisitingMarkId : 0
      )
