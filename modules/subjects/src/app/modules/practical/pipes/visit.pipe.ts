...
export class VisitPipe implements PipeTransform {
  transform(
    value: StudentMark,
    ...args: [string, Practical[]]
  ): PracticalVisitingMark[] {
    const practical = args[1].find(
      (res) => res.PracticalId.toString() === args[0].toString()
    )
    const scheduleIds = []
    ...
    return value.PracticalVisitingMark.filter(
      (visiting) =>
        scheduleIds.includes(
          visiting.ScheduleProtectionPracticalId.toString()
        ) && visiting.Mark
    )
  }
}
