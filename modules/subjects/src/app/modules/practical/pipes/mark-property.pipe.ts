...
export class MarkPropertyPipe implements PipeTransform {
  ...
  ): { mark: PracticalMark; recommendedMark: string } {
    const markProperty = { mark: null, recommendedMark: null }
    const mark = value.PracticalsMarks.find(
    ...
    if (mark && mark.Mark) {
      ...
      for (let i = 0; i < schedule.length; i++) {
        ...
        const oldDate = new Date(
          oldDateArr[1] + '.' + oldDateArr[0] + '.' + oldDateArr[2]
        )
        const now = new Date()
        now.setHours(0, 0, 0, 0)
        if (oldDate.valueOf() === now.valueOf()) {
          const practical = practicals.find(
            (practical) => practical.PracticalId === practicalId
          )
          const scheduleProtectionPractical =
            practical.ScheduleProtectionPracticalsRecommended.find(
              (res) =>
                res.ScheduleProtectionId ===
                calendar.ScheduleProtectionPracticalId
            )
