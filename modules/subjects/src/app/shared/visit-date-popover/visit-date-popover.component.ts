...
export class VisitDatePopoverComponent {
  ...
  dateForm: FormGroup = new FormGroup({
    ...
    lecturerId: new FormControl(null, [
      Validators.required,
      this.validateLector.bind(this),
    ]),
  })
  ...
  deletePopover(day: any) {
    const dialogData: DialogData = {
      ...
      body: this.translate.transform(
        'text.schedule.date.and.connected.data',
        'дату и все связанные с ней данные'
      ),
  ...
  validateLector(control: AbstractControl) {
    if (
      !control.value ||
      !this.lectors.some((x) => x.LectorId == +control.value)
    ) {
