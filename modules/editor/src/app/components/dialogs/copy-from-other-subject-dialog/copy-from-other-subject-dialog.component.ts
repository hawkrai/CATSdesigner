...
export class CopyFromOtherSubjectDialogComponent implements OnInit {
  ...
  constructor(public dialogRef: MatDialogRef<CopyFromOtherSubjectDialogComponent>,
  ...
  ngOnInit() {
    ...
    this._bookService.getAllUserMainDocuments(currentUser ? currentUser.id : 1, currentSubject ? currentSubject.id : 1).subscribe(data => {
      this.documents = data;
