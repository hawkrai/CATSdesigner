...
import { TestQuestion } from "src/app/models/tests/TestQuestion";
declare var $: any;
  ...
  ngOnInit() {
    ...
    this.testService.getTestById(this.testId)
      .pipe(
        ...
        switchMap((test) => this.testService.getNextQuestion(this.testId, this.questionNumber, !test.ForSelfStudy)),
  ...
  public nextQuestion(answer: any, questionNumber?): void {
    if (this.questionArray.length !== 0) {
      ...
      if (this.questionArray.length !== 0) {
        ...
        this.testService.getNextQuestion(this.testId, this.questionNumber, !this.test.ForSelfStudy)
