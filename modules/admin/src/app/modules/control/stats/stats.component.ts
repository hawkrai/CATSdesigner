...
export class StatsComponent implements OnInit {
  ...
  getParamIdFromUrl() {
    this.route.params
      .pipe(
        flatMap(({ groupName }) =>
          forkJoin([this.subjectService.getSubjects(groupName)])
        )
      )
      .subscribe(([subject]) => {
    ...
    if (this.isLoad) {
      if (id && id !== -1) {
        ...
        this.tableStats = this.studentStatistic.map((item) => {
          const userLabPass = item.UserLabPass.find(
            ({ Key }) => Key === id
          ).Value
          const userLecturePass = item.UserLecturePass.find(
            ({ Key }) => Key === id
          ).Value
          const userAvgLabMarks = item.UserAvgLabMarks.find(
            ({ Key }) => Key === id
          ).Value
          const userAvgTestMarks = item.UserAvgTestMarks.find(
            ({ Key }) => Key === id
          ).Value
          return {
            GroupName: groupName,
            ...
            UserTestCount: item.UserTestCount.find(({ Key }) => Key === id)
              .Value,
