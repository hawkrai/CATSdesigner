...
export class ProfileComponent implements OnInit {
  ...
  getDpString() {
    if (this.isStudent()) {
      return this.translatePipe.transform(
        'text.adminPanel.profile.dp',
        'Дипломный проект пользователя'
      )
    } else {
      return this.translatePipe.transform(
        'text.adminPanel.profile.dp.plural',
        'Дипломные проекты пользователя'
      )
    }
  }
  ...
  getProfileInfoSubjects(id: any) {
    this.profileService
      ...
      .subscribe((res: any) => {
        ...
        this.profileInfoSubjects = res.filter(
          (subject: any) => !subject.IsArchive
        )
  ...
  sortProjects(array) {
    return Array.from(this.groupBy(array, (x) => x.SubjectName).entries())
      ...
      .map((x) =>
        x[1].sort((a, b) => this.compareByKey(a, b, (x) => x.Name.trim()))
      )
      .reduce((acc, value) => {
        acc.push(...value)
        return acc
