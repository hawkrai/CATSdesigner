  ...
  chartOptions = {
    ...
    tooltip: {
      custom: ({ series, seriesIndex, dataPointIndex, w }) => {
        ...
        let times = this.logins
          .get(date)
          .map((val) => this.formatTime(new Date(val)))

        const html = `<div class="tooltip" style="margin-right:20px">
                <p style="margin-left:20px"><strong>${this.translatePipe.transform(
                  'text.adminPanel.modal.statistic.count',
                  ''
                )}:</strong> ${times.length}</p>
                <p style="margin-left:20px;line-height:0px"><strong>${this.translatePipe.transform(
                  'text.adminPanel.modal.statistic.times',
                  ''
                )}:</strong></p>
                <ol>
                  ${times.map((val) => `<li>${val}</li>`).join('\n')}
                </ol>
              </div>`
  ...
  initChart() {
    ...
    for (const item of this.logins.entries()) {
      this.series.data.push({
        x: this.formatDate(new Date(item[0])),
        y: item[1].length,
      })
    }
