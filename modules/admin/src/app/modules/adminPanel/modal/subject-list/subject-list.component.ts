...
export class SubjectListComponent implements OnInit {
  ...
  getSubjectStatus(status) {
    if (status == true) {
      return this.transaltePipe.transform(
        'text.adminPanel.modal.subjectList.ongoing',
        'Текущий'
      )
    } else {
      return this.transaltePipe.transform(
        'text.adminPanel.modal.subjectList.completed',
        'Завершенный'
      )
    }
  }
  ...
  loadData(data) {
    if (!this.isGroup(data)) {
      this.userService
        .getListOfAllSubjectsByStudentId(data.Id)
        .subscribe((result) => {
      ...
      this.userService
        .getListOfAllSubjectsByGroupId(data.Id)
        .subscribe((result) => {
          this.subjectInfo = result
          this.isLoad = true
        })
