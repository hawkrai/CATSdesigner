    ...
    this.studentService.editStudents(student).subscribe(() => {
      ...
      this.toastr.addSuccessFlashMessage(
        this.translatePipe.transform(
          'text.adminPanel.students.edit.success',
          ''
        )
      )
    })
  ...
  restoreStudent(id): void {
    this.studentService.restoreStudent(id).subscribe(
      () => {
        ...
        this.toastr.addSuccessFlashMessage(
          this.translatePipe.transform(
            'text.adminPanel.students.restore.success',
            ''
          )
        )
      },
      () => {
        this.toastr.addErrorFlashMessage(
          this.translatePipe.transform(
            'text.adminPanel.students.restore.error',
            ''
          )
        )
      }
      ...
      if (result) {
        this.studentService.deleteStudent(id).subscribe(
          () => {
            ...
            this.toastr.addSuccessFlashMessage(
              this.translatePipe.transform(
                'text.adminPanel.students.delete.success',
                ''
              )
            )
          },
          (err) => {
            this.toastr.addErrorFlashMessage(
              this.translatePipe.transform(
                'text.adminPanel.students.delete.error',
                ''
              )
            )
          }
  ...
  getStudentStatus(student) {
    if (!student.IsActive) {
      return this.translatePipe.transform(
        'text.adminPanel.students.status.deleted',
        ''
      )
    }
  ...
  getStatusTooltip(student: Student) {
    ...
    if (!student.IsActive) {
      if (student.DeletedOn != null) {
        return (
          this.translatePipe.transform(
            'text.adminPanel.students.tooltips.deletedOn',
            ''
          ) +
          '\r\n' +
    ...
    if (
      ...
      return (
        this.translatePipe.transform(
          'text.adminPanel.students.tooltips.confirmedOn',
          ''
        ) +
        '\r\n' +
    ...
    if (
      ...
      return (
        this.translatePipe.transform(
          'text.adminPanel.students.tooltips.confirmationCancelledOn',
          ''
        ) +
        '\r\n' +
        `${this.formatDate(student.ConfirmationDate)}\r\n` +
