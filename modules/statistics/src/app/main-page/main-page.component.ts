          ...
          this.serviceService.getGroupsBySubjectId(subject.SubjectId).subscribe(groups => {
            this.serviceService.getCheckedType(subject.SubjectId).subscribe(types => {
              ...
              if (res.SubjectStatistics.indexOf(subject) == res.SubjectStatistics.length - 1) {
                ...
                this.seriesArchive.push({name: this.categoriesConst[0], data: this.practMarksArchive});
                this.seriesArchive.push({name: this.categoriesConst[1], data: this.labMarksArchive});
                this.seriesArchive.push({name: this.categoriesConst[2], data: this.testMarksArchive});
                this.seriesArchive.push({name: this.categoriesConst[3], data: this.conMarksArchive});
                this.seriesArchive.push({name: this.categoriesConst[4], data: this.ratingMarksArchive});
      ...
      this.serviceService.getCheckedType(subject.Id).subscribe(types => {
        ...
        result.Students.forEach(student => {
          if (student.Id == this.user.id) {
            if (student.UserAvgTestMarks.find(({Key}) => Key === subject.Id) != undefined) {
