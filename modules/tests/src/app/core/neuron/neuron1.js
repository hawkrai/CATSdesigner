...
export var neuralNetworkV2 = {
  ...
  formatData: function (data) {
    ...
    if (!_.isArray(datum) && !(datum instanceof Float64Array)) {
      if (!this.inputLookup) {
        this.inputLookup = buildLookup(_.pluck(data, 'input'))
      }
      data = data.map(function (datum) {
        var array = toArray(this.inputLookup, datum.input)
        return _(_.clone(datum)).extend({ input: array })
      }, this)
    }
    ...
    if (!_.isArray(data[0].output)) {
      ...
      data = data.map(function (datum) {
        ...
        return _(_.clone(datum)).extend({ output: array })
      }, this)
    }
    return data
  },
