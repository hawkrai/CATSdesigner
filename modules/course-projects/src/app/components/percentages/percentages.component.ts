...
export class PercentagesComponent implements OnInit {
    ...
    const dialogRef = this.dialog.open(AddStageDialogComponent, {
      ...
      data: {
        title: this.translatePipe.transform(
          'text.course.percentages.dialog.title.add',
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != null && result.name != null) {
        ...
        if (checkTheme === undefined) {
            ...
            .subscribe(() => {
              ...
              this.toastr.success(
                this.translatePipe.transform(
                  'text.course.percentages.dialog.save.success',
                  'Этап успешно сохранен'
                )
              )
            })
        } else {
          this.toastr.error(
            this.translatePipe.transform(
              'text.course.percentages.dialog.save.error',
  ...
  editStage(stage: Percentage, itemIndex: number) {
    const dialogRef = this.dialog.open(AddStageDialogComponent, {
      ...
      data: {
        title: this.translatePipe.transform(
          'text.course.percentages.dialog.title.edit',
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result != null && result.name != null) {
        result.name = result.name.replace('\n', '')
        const checkTheme = this.percentages.find(
          (i, index) =>
            i.Name === result.name &&
            i.Percentage === +result.percentage &&
            index !== itemIndex
        )
        const checkName = this.percentages.find(
          (i, index) => i.Name === result.name && index !== itemIndex
        )
        if (checkTheme === undefined && !checkName) {
          ...
          this.percentagesService
            ...
            .subscribe(() => {
              ...
              this.toastr.success(
                this.translatePipe.transform(
                  'text.course.percentages.dialog.edit.success',
