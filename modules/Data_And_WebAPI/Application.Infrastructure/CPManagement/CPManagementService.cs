...
namespace Application.Infrastructure.CPManagement
{
    public class CPManagementService: ICPManagementService
    {
        public PagedList<CourseProjectData> GetProjects(int userId, GetPagedListParams parms)
        {
            ...
            IQueryable<CourseProject> query = Context.CourseProjects.AsNoTracking()
            ...
            if (searchString.Length > 0)
            {
                ...
                return courseProjects.ApplyPaging(parms);//TODO
            }
            else
            {
                var courseProjects = from cp in query
        ...
        {
            var newses = Context.CourseProjectNews.Where(x => x.SubjectId == subjectId).OrderByDescending(x=>x.EditDate);
        ...
        public CourseProjectNews SaveNews(CourseProjectNews news, IList<Attachment> attachments, int userId)
        {
            ...
            var cpEditNews = Context.CourseProjectNews.FirstOrDefault(x => x.Id == news.Id && x.SubjectId == news.SubjectId);
            if (cpEditNews != null)
            {
        ...
        {
           var models = Context.CourseProjectNews.Where(e => e.SubjectId == subjectId);
        ...
        {
            return Context.CourseProjectNews.Single(x => x.Id == id);
        }
