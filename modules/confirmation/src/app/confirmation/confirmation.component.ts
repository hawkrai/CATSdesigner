...
export class ConfirmationComponent implements OnInit, OnDestroy {
  ...
  public confirm(student: Student, confirm = true): void {
    ...
    iif(
      () => confirm,
      this.confirmationService.confirmationStudent(student.StudentId),
      this.confirmationService.unconfirmationStudent(student.StudentId)
    )
      .pipe(takeUntil(this.unsubscribeStream$))
      .subscribe(
        (response) => {
          ...
          this.catsMessageService.sendMessage(
            new Message('Confirmation', confirm ? '-1' : '1')
          )
          this.catsMessageService.sendMessage(
            new Message(
              'Toast',
  ...
  private updateGroupUncofirmedStudents(increase: boolean = true): void {
    this.groups = this.groups.map(
      (group): Group =>
        +group.GroupId === +this.selectedGroup
          ? {
              ...group,
              CountUnconfirmedStudents:
                group.CountUnconfirmedStudents + (increase ? -1 : 1),
            }
          : group
    )
  }
