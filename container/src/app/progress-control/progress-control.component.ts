...
export class ProgressControlComponent implements OnInit {
  ...
  constructor(
    ...
    this.dateAdapter.setLocale(
      this.translatePipe.transform('text.schedule.locale.en', 'ru')
    )
  }

  ngOnInit(): void {
    this.selectedGroup =
      this.sanitizer.bypassSecurityTrustResourceUrl(`/control/main/0`)
  ...
  enterGroup(): void {
    if (
      (this.f.start.value != '' && this.f.end.value == '') ||
      (this.f.start.value == '' && this.f.end.value != '')
    ) {
      return
    }
