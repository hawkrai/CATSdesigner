...
import {
  ...
  User,
  Group,
} from '../../models/searchResults/search-results'
...
export class SearchService {
  ...
  getLecturerSearchResults(searchString: any): Observable<Lecturer[]> {
    return this.http.get<Lecturer[]>(
      this.api + 'GetLecturerSearchResult/' + searchString
    )
  }

  getStudentSearchResults(searchString: any): Observable<Student[]> {
    return this.http.get<Student[]>(
      this.api + 'GetStudentSearchResult/' + searchString
    )
  }

  getGroupSearchResults(searchString): Observable<Group[]> {
    return this.http.get<Group[]>(
      this.api + 'GetGroupSearchResult/' + searchString
    )
  }
}
