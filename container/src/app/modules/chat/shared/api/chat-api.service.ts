...
export class ChatApiService {
  ...
  getChatById(userId: number, chatId: number): Observable<any> {
    return this.http.get<any>(`${this.baseUrl}/GetChatById?userId=${userId}&chatId=${chatId}`);
  }

  updateReadChat(userId: number, chatId: number): Observable<Chat[]> {
    return this.http.get<Chat[]>(`${this.baseUrl}/UpdateReadChat?userId=${userId}&chatId=${chatId}`);
  }

  updateReadGroupChat(userId: number, chatId: number): Observable<Chat[]> {
    return this.http.get<Chat[]>(`${this.baseUrl}/UpdateReadGroupChat?userId=${userId}&chatId=${chatId}`);
  }

  getAllGroups(userId: number, role: string): Observable<SubjectGroups[]> {
    return this.http.get<SubjectGroups[]>(`${this.baseUrl}/GetAllGroups?userId=${userId}&role=${role}`);
  }

  getAllLecturers(filter: string, limit: number, offset: number): Observable<User[]> {
    return this.http.get<User[]>(`${this.baseUrl}/GetAllLecturers?filter=${filter}&limit=${limit}&offset=${offset}`);
  }
