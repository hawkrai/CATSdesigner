...
export class ChangePersonalDataComponent implements OnInit {
  ...
  dataGroup = new FormGroup({
    emailFormControl: new FormControl('', [
      Validators.pattern(
        '^([A-Za-z0-9_.-]{1,30}@[A-Za-z0-9_.-]{1,30}[.]{1}[A-Za-z0-9_-]{1,30})$'
      ),
    ]),
    ...
    reader.onloadend = function () {
      if (buffer.isGoodAvatarImage(reader.result.toString())) {
        if (reader.result.toString().length < buffer.MAX_IMAGE_LEN) {
          ...
          buffer.toastr.addWarningFlashMessage(
            buffer.translatePipe.transform(
              'text.personalAccount.tooBigImage',
        ...
        buffer.toastr.addWarningFlashMessage(
          buffer.translatePipe.transform(
            'text.personalAccount.wrongImage',
  ...
  updatePersonalInfo() {
    this.trimFields()
    if (
      !this.dataGroup.controls.patronymicFormControl.invalid &&
      !this.dataGroup.controls.surnameFormControl.invalid &&
      !this.dataGroup.controls.nameFormControl.invalid &&
      !this.dataGroup.controls.phoneFormControl.invalid &&
      !this.dataGroup.controls.emailFormControl.invalid
    ) {
      this.dataService
        .changeProfileData(this.profileData, this.profileData.Avatar)
        .subscribe((res) => {
          if (res) {
            this.toastr.addSuccessFlashMessage(
              this.translatePipe.transform(
                'text.personalAccount.changesSaved',
            ...
            this.toastr.addErrorFlashMessage(
              this.translatePipe.transform(
                'text.personalAccount.changesNotSaved',
      ...
      this.toastr.addWarningFlashMessage(
        this.translatePipe.transform(
          'text.personalAccount.wrongData',
          'Некоторые поля не соответствуют формату!'
        )
      )
    }
  }
  ...
  deleteSpaces() {
    ...
    if (this.profileData.SkypeContact != null)
      this.profileData.SkypeContact = this.delEmptyRows(
        this.profileData.SkypeContact
      )
  }
